...
*global root="C:\Users\hrajaiya\Dropbox\IPO_CSR"
...
*match NETS CSR list with VC-backed firms and keep the non-VC backed firms
	...
	use $bt/pm_identifiers_all_names_std, clear
			...
			keep original_name reprisk_id stem_name upper_name standard_name all_name_listed
			gen uniq_repr_id=_n
	...
	*** check the match rate
	...
	keep uniq_repr_id reprisk_id stem_name ipo_deal_id issuer_stem similscore upper_name original_name I TIC
	duplicates drop upper_name, force
	...
	*** check the match rate
	...
	keep uniq_repr_id reprisk_id stem_name ma_deal_id target_stem similscore upper_name original_name TN
	duplicates drop upper_name, force
	...
	*** track acquisition and IPO date of sample firms
	   ...
	   *** track acquisitions
		...
		*** filter unique non-VC firms
		...
		keep uniq_repr_id reprisk_id upper_name original_name_csr status sectors 
		...
		**** match non-VC backed firms with Crunchbase data
		...
		keep uniq_repr_id reprisk_id upper_name original_name_csr standard_name stem_name  status sectors
		...
		*** manual check
		...
		keep if (sim==1) | (sim>0.99 & score>0.6) | (score>0.7) | (score>0.65 & sim>0.98)
		 ...
		 duplicates drop uniq_repr_id, force
	   ...
	   *** list of VC-backed firms that received first VC investment after the CSR ratings
		...
		*** labels
		...
		save $bo/vc_csr_reg_sample, replace
		...
		*** 1-year
		...
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding1.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		*** 3-year
		...
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding3.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		*** 5-year
		...
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding5.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		*** any point in future
		...
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		**** summary stats
		...
		est store e5
			...
			outreg2 [e1 e2 e3 e4 e5] using $br/IPO_funding.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		est store e5
			...
			outreg2 [e1 e2 e3 e4 e5] using $br/M&A_funding.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		outreg2 [e1 e2 e3 e4 e5] using $br/Exit_funding.xls, keep (avg_reprisk_rating_year avg_current_rri_year avg_environmental_rri_year avg_social_rri_year avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		*********  x-sectional tests
		...
		est store e5
			...
			outreg2 [e1 e2 e3 e4 e5] using $br/IPO_funding_xsection.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		est store e5
			...
			outreg2 [e1 e2 e3 e4 e5] using $br/M&A_funding_xsection.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace	
		...
		est store e5
			...
			outreg2 [e1 e2 e3 e4 e5] using $br/Exit_funding_xsection.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace	
			
			*************************
			
*check with harshit (create change in rating)
use $bt/pm_rri_US_nodup, clear
	...
	gen governance_rri = current_rri * governance_percentage
		...
		bys reprisk_id year: egen avg_`v'_year = mean(`v')
		bys reprisk_id year: egen max_`v'_year = max(`v')
		}
	...
	bys reprisk_id year: egen max_reprisk_rating_year = max(numerical_reprisk_rating)
	bys reprisk_id year: egen min_reprisk_rating_year = min(numerical_reprisk_rating)
	bys reprisk_id year: egen avg_reprisk_rating_year = mean(numerical_reprisk_rating)
		
	
	keep original_name_csr  year max_* min_* avg* sectors   reprisk_id
	duplicates drop
		destring reprisk_id, gen(reprisk_id_num)
		tsset reprisk_id_num year
		foreach v in environmental_rri social_rri governance_rri current_rri {
		gen dt_avg_`v'_year = f1.avg_`v'_year - avg_`v'_year
		gen dt_max_`v'_year = f1.max_`v'_year - max_`v'_year
		}	
		gen dt_avg_reprisk_rating_year = f1.avg_reprisk_rating_year - avg_reprisk_rating_year
		gen dt_max_reprisk_rating_year = f1.max_reprisk_rating_year - max_reprisk_rating_year
		gen dt_min_reprisk_rating_year = f1.min_reprisk_rating_year - min_reprisk_rating_year		
		drop reprisk_id_num
save $bt/pm_rri_US_year_nodup1, replace				
			
				
use  $bo/vc_csr_reg_sample, clear
		drop max_* min_* avg*
		merge 1:1 reprisk_id year using $bt/pm_rri_US_year_nodup1
			keep if _merge == 3
			drop _merge
		
				*** labels
		label var dt_avg_current_rri_year "Change in Average Monthly Current RepRisk Index (RRI) (Annual)"
		label var dt_avg_environmental_rri_year "Change in Average Monthly Enviromental RepRisk Index (RRI) (Annual)"
		label var dt_avg_social_rri_year "Change in Average Monthly Social RepRisk Index (RRI) (Annual)"
		label var dt_avg_governance_rri_year "Change in Average Monthly Governance RepRisk Index (RRI) (Annual)"
		label var max_current_rri_year "Maximum Monthly Current RepRisk Index (RRI) (Annual)"
		label var max_environmental_rri_year "Maximum Monthly Enviromental RepRisk Index (RRI) (Annual)"
		label var max_social_rri_year "Maximum Monthly Social RepRisk Index (RRI) (Annual)"
		label var max_governance_rri_year "Maximum Monthly Governance RepRisk Index (RRI) (Annual)"		
		label var max_reprisk_rating_year "Best Monthly RepRisk Rating (Annual)"		
		label var min_reprisk_rating_year "Worst Monthly RepRisk Rating (Annual)"		
		label var dt_avg_current_rri "Change in Average Monthly Current RepRisk Index (RRI) All Time"
		label var dt_avg_environmental_rri "Change in Average Monthly Enviromental RepRisk Index (RRI) All Time"
		label var dt_avg_social_rri "Change in Average Monthly Social RepRisk Index (RRI) All Time"
		label var dt_avg_governance_rri "Change in Average Monthly Governance RepRisk Index (RRI) All Time"
		label var max_current_rri "Maximum Monthly Current RepRisk Index (RRI) All Time"
		label var max_environmental_rri "Maximum Monthly Enviromental RepRisk Index (RRI) All Time"
		label var max_social_rri "Maximum Monthly Social RepRisk Index (RRI) All Time"
		label var max_governance_rri "Maximum Monthly Governance RepRisk Index (RRI) All Time"		
		label var max_reprisk_rating "Best Monthly RepRisk Rating All Time"		
		label var min_reprisk_rating "Worst Monthly RepRisk Rating All Time"	
		label var dt_avg_reprisk_rating "Change in Average RepRisk Rating (Annual)"		
		****** Regressions
		
		*** 1-year
		reghdfe vc_fund_1year dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe vc_fund_1year dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe vc_fund_1year dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe vc_fund_1year dt_avg_governance_rri_year, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe vc_fund_1year dt_avg_current_rri_year, absorb (year ind) cluster(ind)	
		est store e5
		
		
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding1_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace

		*** 3-year
		reghdfe vc_fund_3year dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe vc_fund_3year dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe vc_fund_3year dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe vc_fund_3year dt_avg_governance_rri_year, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe vc_fund_3year dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		est store e5
		
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding3_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		
		*** 5-year
		reghdfe vc_fund_5year dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe vc_fund_5year dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe vc_fund_5year dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe vc_fund_5year dt_avg_governance_rri_year, absorb (year ind) cluster(ind)
		est store e4
		reghdfe vc_fund_5year dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		est store e5
		
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding5_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		
		*** any point in future
		
		reghdfe vc_backed dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe vc_backed dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe vc_backed dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe vc_backed dt_avg_governance_rri_year, absorb (year ind) cluster(ind)
		est store e4
		reghdfe vc_backed dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		est store e5
		
		outreg2 [e1 e2 e3 e4 e5] using $br/VC_funding_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		...
		*** exit within 5 years
		...
		reghdfe ipo_5 dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		reghdfe ipo_5 dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		reghdfe ipo_5 dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		reghdfe ipo_5 dt_avg_governance_rri_year, absorb (year ind) cluster(ind)			
		reghdfe ipo_5 dt_avg_current_rri_yea, absorb (year ind) cluster(ind)	
				
		reghdfe ma_5 dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		reghdfe ma_5 dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		reghdfe ma_5 dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		reghdfe ma_5 dt_avg_governance_rri_year, absorb (year ind) cluster(ind)			
		reghdfe ma_5 dt_avg_current_rri_yea, absorb (year ind) cluster(ind)	
		
		reghdfe exit_5 dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		reghdfe exit_5 dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		reghdfe exit_5 dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		reghdfe exit_5 dt_avg_governance_rri_year, absorb (year ind) cluster(ind)			
		reghdfe exit_5 dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		...
		**** summary stats
		univar dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year vc_backed ipo ma exit
		
		
		reghdfe ipo dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe ipo dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe ipo dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe ipo dt_avg_governance_rri_year, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe ipo dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		est store e5
		
			outreg2 [e1 e2 e3 e4 e5] using $br/IPO_funding_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
				
		reghdfe ma dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe ma dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe ma dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe ma dt_avg_governance_rri_year, absorb (year ind) cluster(ind)
		est store e4
		reghdfe ma dt_avg_current_rri_yea, absorb (year ind) cluster(ind)	
		est store e5
		
			outreg2 [e1 e2 e3 e4 e5] using $br/M&A_funding_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		
		reghdfe exit dt_avg_reprisk_rating_year, absorb (year ind) cluster(ind)
		est store e1
		reghdfe exit dt_avg_environmental_rri_year, absorb (year ind) cluster(ind)
		est store e2
		reghdfe exit dt_avg_social_rri_year, absorb (year ind) cluster(ind)
		est store e3
		reghdfe exit dt_avg_governance_rri_year, absorb (year ind) cluster(ind)
		est store e4
		reghdfe exit dt_avg_current_rri_yea, absorb (year ind) cluster(ind)
		est store e5
		
		outreg2 [e1 e2 e3 e4 e5] using $br/Exit_funding_delta.xls, keep (dt_avg_reprisk_rating_year dt_avg_current_rri_year dt_avg_environmental_rri_year dt_avg_social_rri_year dt_avg_governance_rri_year) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		
		*********  x-sectional tests
		
				
		reghdfe ipo c.dt_avg_reprisk_rating_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e1
		reghdfe ipo c.dt_avg_environmental_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e2
		reghdfe ipo c.dt_avg_social_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e3
		reghdfe ipo c.dt_avg_governance_rri_year##c.vc_backed, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe ipo c.dt_avg_current_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e5
		
			outreg2 [e1 e2 e3 e4 e5] using $br/IPO_funding_xsection_delta.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace
		
					
		reghdfe ma c.dt_avg_reprisk_rating_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e1
		reghdfe ma c.dt_avg_environmental_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e2
		reghdfe ma c.dt_avg_social_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e3
		reghdfe ma c.dt_avg_governance_rri_year##c.vc_backed, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe ma c.dt_avg_current_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e5
		
			outreg2 [e1 e2 e3 e4 e5] using $br/M&A_funding_xsection_delta.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace	
		
		reghdfe exit c.dt_avg_reprisk_rating_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e1
		reghdfe exit c.dt_avg_environmental_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e2
		reghdfe exit c.dt_avg_social_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e3
		reghdfe exit c.dt_avg_governance_rri_year##c.vc_backed, absorb (year ind) cluster(ind)	
		est store e4
		reghdfe exit c.dt_avg_current_rri_year##c.vc_backed, absorb (year ind) cluster(ind)
		est store e5
		
			outreg2 [e1 e2 e3 e4 e5] using $br/Exit_funding_xsection_delta.xls, drop (constant) adjr2 dec(3) addtext(Industry FE, Yes, Year FE, Yes) word seeout label excel replace	
